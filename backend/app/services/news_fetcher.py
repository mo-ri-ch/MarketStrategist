...
def get_fallback_news(competitor_name: str, region: str = None) -> List[Dict[str, Any]]:
    """
    ...
    region_str = f" in the {region} region" if region and region.lower() != "global" else ""
    
    templates = [
        {
            "title": f"{competitor_name} Raises $45M in Series C Funding to Accelerate Enterprise AI Platform",
            "description": f"{competitor_name} announced today that it has secured $45 million in Series C funding{region_str}. The round was led by Sequoia Capital, with participation from existing investors. The new capital will be used to scale the company's enterprise AI platform and expand its engineering team.",
            "source": "TechCrunch",
        ...
        },
        {
            ...
            "description": f"In a strategic move to boost its data intelligence offerings{region_str}, {competitor_name} has completed the acquisition of DataAnalytics Corp, a developer of advanced forecasting tools. The integration is expected to bring immediate real-time reporting enhancements to users.",
            "source": "VentureBeat",
        ...
        },
        {
            ...
            "description": f"Google Cloud and {competitor_name} today announced a multi-year partnership to host and scale cloud operations{region_str}. Under the agreement, {competitor_name} will migrate its core workloads to Google Cloud and collaborate on joint go-to-market solutions.",
            "source": "Reuters",
        ...
        },
        {
            ...
            "description": f"Responding to growing market demand for real-time strategy models{region_str}, {competitor_name} unveiled its new predictive engine. The module promises up to 3x faster response times and seamless API integration for enterprise subscribers.",
            "source": "Forbes",
...
def fetch_competitor_news(competitor_name: str, region: str = None) -> List[Dict[str, Any]]:
    """
    ...
    logger.info(f"Simulating news fetch for competitor: {competitor_name} (Region: {region_str})")
    
    client = get_openai_client()
    
    if not client:
        return get_fallback_news(competitor_name, region)
